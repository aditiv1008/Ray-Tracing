...
class Sphere:
    ...
    def intersect(self, ray):
        ...
        if disc >= 0:
          ...
          t = -1
          if (ray.start <= t1 <= ray.end) and (ray.start <= t2 <= ray.end):
              t = min(t1,t2)
          elif (ray.start <= t1 <= ray.end) and (not ray.start <= t2 <= ray.end):
              t = t1
          elif (not ray.start <= t1 <= ray.end) and (ray.start <= t2 <= ray.end):
              t = t2
          if t!=-1:
              point = p+self.center+t*d
...
class Triangle:
    ...
    def intersect(self, ray):
        ...
        # TODO A4 implement this function
        a, b, c = self.vs
        A = np.array([a - b, a - c, ray.direction]).T
        beta, gamma, t = np.linalg.solve(A, a - ray.origin)
        if 0 < beta < 1 and 0 < gamma < 1 and beta + gamma < 1 and ray.start < t < ray.end:
...
class Camera:
    ...
    def __init__(self, eye=vec([0,0,0]), target=vec([0,0,-1]), up=vec([0,1,0]), 
        ...
        self.f = np.linalg.norm(target - self.eye)  # You should set this to the distance from your center of projection to the image plane
        self.height =  2*self.f*abs(np.tan(vfov/2 * np.pi/180))
        self.width = abs(self.aspect * self.height)
        self.M = np.array([[self.width, 0, -self.width/2], [0, -self.height, self.height/2], [0, 0, 1]])   # Set this to the matrix that transforms your camera's coordinate system to world coordinates

        # TODO A4 implement this constructor to store whatever you need for ray generation
        self.w = -normalize(target - eye) #-d
    ...
    def generate_ray(self, img_point):
        ...
        """
        # TODO A4 implement this function
        img_point_transformed = np.dot(self.M, np.array([img_point[0], img_point[1], 1]))

        origin = self.eye
        # + (img_point_transformed[0] * self.u) + (img_point_transformed[1] * self.v)
        direction = -self.w*self.f+ (img_point_transformed[0] * self.u) + (img_point_transformed[1] * self.v)
        # print("origin:",origin)
...
class PointLight:
    ...
    def illuminate(self, ray, hit, scene):
        ...
        shad_ray = Ray(hit.point, self.position - hit.point)
        shad_hit = scene.intersect(shad_ray)
        if shad_hit is no_hit or shad_hit.t > np.linalg.norm(self.position - hit.point):
            v = -normalize(ray.direction)
            l = normalize(self.position - hit.point)
            cos_theta = max(0, np.dot(hit.normal, l))
            r = np.linalg.norm(self.position - hit.point)
            R = 0.0
            #specular
            h = normalize(v+l)

            L_d = (hit.material.k_d + (hit.material.k_s * np.dot(hit.normal, h)**hit.material.p)) * (max(0, np.dot(hit.normal, l)) / r**2) * self.intensity
            return L_d
...
def shade(ray, hit, scene, lights, depth=0):
    ...
    if hit is not no_hit:
        result = lights[0].illuminate(ray, hit, scene)
        #mirror reflection
        if depth < MAX_DEPTH and hit.material.k_m > 0:
            v = -normalize(ray.direction)
            reflect = 2*np.dot(hit.normal, v) * hit.normal - v
            reflect_ray = Ray(hit.point, reflect)
            result += hit.material.k_m * shade(reflect_ray, scene.intersect(reflect_ray), scene, lights, depth + 1)
        for light in lights[1:]:
            result += light.illuminate(ray, hit, scene)
        return result
...
def render_image(camera, scene, lights, nx, ny):
    ...
    for i in range(ny):
        for j in range(nx):
            origin = [0,0,0] + (2*(i+0.5)/ny-1)*np.array([1,0,0],np.float64) + (2*(j+0.5)/nx-1)*np.array([0,1,0],np.float64)
            direction = [0,0,-1]
            # Generate Ray---we recommend just generating an orthographic ray to start with
